...
async def get_friends_not_in_challenge(
    ...
    friends_map = await check_friendship_status(session, current_user, all_user_ids)
    friends_ids = list(friends_map.keys())
    if not friends_ids:
        return []
    ...
    return [FriendProfileRead.model_validate(u) for u in users]
