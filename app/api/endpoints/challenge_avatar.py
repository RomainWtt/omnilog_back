...
@router.post("/{challenge_id}/avatar", response_model=ChallengeRead)
...
@router.delete("/{challenge_id}/avatar", status_code=status.HTTP_204_NO_CONTENT)
async def delete_challenge_avatar(
    challenge_id: UUID,
    current_user: User = Depends(get_current_active_user),
    session: AsyncSession = Depends(get_session)
):
    """Delete challenge avatar. Only creator or admin can delete."""
    challenge = await get_challenge_by_id(session, challenge_id)
    if not challenge:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
